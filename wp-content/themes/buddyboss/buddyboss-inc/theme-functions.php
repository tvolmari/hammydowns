...
function buddyboss_setup()
{
	...
	add_image_size( 'buddyboss_pic_med', 501, 9999 );
	add_image_size( 'buddyboss_pic_wide', 750, 9999 );
	add_image_size( 'buddyboss_pic_large', 1300, 9999 );
}
add_action( 'after_setup_theme', 'buddyboss_setup' );

/**
 * Stops WooCommerce from hiding the admin bar
 *
 * @since  BuddyBoss 3.1.8
 * @return boolean false
 */
function buddyboss_stop_woocommerce_adminbar_filter()
{
	return false;
}
add_filter( 'woocommerce_disable_admin_bar', 'buddyboss_stop_woocommerce_adminbar_filter' );
...
function buddyboss_load_fonts()
{
	if ( !is_admin() ) {
        ...
        wp_register_style('googlefonts', "http" . ($_SERVER['SERVER_PORT'] == 443 ? "s" : "") . "://fonts.googleapis.com/css?family=Open+Sans:300italic,400italic,600italic,700italic,400,700,600,300", false, null);
...
function buddyboss_scripts_styles()
{
	...
	if ( $buddyboss->buddypress_active && ! is_admin() )
	{
		...
		$bp_js_dependencies = function_exists( 'bp_core_get_js_dependencies' )
