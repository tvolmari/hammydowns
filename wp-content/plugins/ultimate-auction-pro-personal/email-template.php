	...
	if ($check_method === 'method_paypal') {
		...
		if (in_array('administrator', $winner_user->roles)) {
			...
			if ($shipAmt > 0) {
				...
				$msg_ship_pp .= __("Shipping Charge", "wdm-ultimate-auction") . ": <strong>" . $cur_code . " " . $shipAmt . "</strong><br /><br />";
			}
...
//email template for offer reject
function ua_best_offers_reject_mail( $auction_name, $auction_id, $auction_desc, $offer_val, $offer_sender_email, $return_url){
	$email_template_details = get_option("wdm_ua_email_template_best_offer_reject", 1);
	$site_name 	= get_bloginfo('name');
	$c_code = substr(get_option('wdm_currency'), -3);
	$email_sent = false;
	//$ua_email_enable = true;
	...
	if (isset($email_template_details['template']) && $email_template_details['template'] == "ua_custom") {
		...
		$subject = str_replace( '{product_name}', $auction_name, $subject );

		$message = str_replace( '{product_url}', $return_url, wpautop(convert_chars(wptexturize($email_template_details['body'] ))));
		$message = str_replace( '{product_name}', $auction_name, $message );
		$message = str_replace( '{currency_code}', $c_code, $message );
		$message = str_replace( '{offer_value}', sprintf("%.2f", $offer_val), $message );
		$message = str_replace( '{product_description}', html_entity_decode(strip_tags($auction_desc)), $message );
	}
	else{
		$subject = "";
		$subject = "[".$site_name."] ".__('Your offer has been rejected', 'wdm-ultimate-auction')." - ".$auction_name;

		$message  = "";
		$message .= __('Your offer has been rejected by the product owner.', 'wdm-ultimate-auction');
		$message .= "<br /><br /> ".__('Here are the details', 'wdm-ultimate-auction')." - ";
	    $message .= "<br /><br /> ".__('Product URL', 'wdm-ultimate-auction').": <a href='".$return_url."'>". $return_url."</a>";
	    $message .= "<br /><br /> ".__('Product Name', 'wdm-ultimate-auction').": ".$auction_name;
	    $message .= "<br /><br /> ".__('Offer Value', 'wdm-ultimate-auction').": ".$c_code." ".sprintf("%.2f", $offer_val);
	    $message .= "<br /><br /> ".__('Description', 'wdm-ultimate-auction').": <br />".html_entity_decode(strip_tags($auction_desc))."<br />";
	}
	...
	$headers = "";
	//$headers  = "From: ". get_bloginfo('name') ." <". $seller_email ."> \r\n";
	$headers .= "Reply-To: <". $offer_sender_email ."> \r\n";
	$headers .= $hdr;
	
	if ($ua_email_enable) {
		$email_sent = wp_mail( $offer_sender_email, $subject, $message, $headers, '' );
	}

	return $email_sent;
}

function ua_place_best_offers_mail($auction_name, $auction_id, $auction_desc, $offer_val, $offer_sender_name, $offer_sender_email, $return_url){

	$email_template_details = get_option("wdm_ua_email_template_best_offer_receive", 1);
	//$ua_email_enable = true;
	$email_sent = false;
	$site_name 	= get_bloginfo('name');
	$c_code = substr(get_option('wdm_currency'), -3);
	...
	if (isset($email_template_details['template']) && $email_template_details['template'] == "ua_custom") {
		...
		$message = str_replace( '{offer_value}', sprintf("%.2f", $offer_val), $message );
		$message = str_replace( '{product_description}', html_entity_decode(strip_tags($auction_desc)), $message );
		$message = str_replace( '{offer_sender_name}', $offer_sender_name, $message );
		$message = str_replace( '{offer_sender_email}', $offer_sender_email, $message );
		...
		$subject = "";
		$subject = "[".$site_name."] ".__('An offer has been received for your product', 'wdm-ultimate-auction')." - ".$auction_name;

		$message  = "";
		$message .= __('An offer has been received from a bidder.', 'wdm-ultimate-auction');
		$message .= "<br /><br /> ".__('Here are the details', 'wdm-ultimate-auction')." - ";
	    $message .= "<br /><br /> ".__('Product URL', 'wdm-ultimate-auction').": <a href='".$return_url."'>". $return_url."</a>";
	    $message .= "<br /><br /> ".__('Product Name', 'wdm-ultimate-auction').": ".$auction_name;
	    $message .= "<br /><br /> ".__('Offer Value', 'wdm-ultimate-auction').": ".$c_code." ".sprintf("%.2f", $offer_val);
	    $message .= "<br /><br /> ".__('Description', 'wdm-ultimate-auction').": <br />".html_entity_decode(strip_tags($auction_desc))."<br />";
	    $message .= "<br /><br /><strong>".__('Offer Sender Details', 'wdm-ultimate-auction')."</strong> - <br /><br />";
		$message .= "&nbsp;&nbsp;".__('Name', 'wdm-ultimate-auction').": ".$offer_sender_name."<br /><br />";
		$message .= "&nbsp;&nbsp;".__('Email', 'wdm-ultimate-auction').": ".$offer_sender_email."<br /><br />";
	}
	...
	if ($ua_email_enable) {
		...
		$headers .= "Reply-To: <". $seller_email ."> \r\n";
		$headers .= $hdr;
		$email_sent = wp_mail( $seller_email, $subject, $message, $headers, '' );
	}
	return $email_sent;
}
?>