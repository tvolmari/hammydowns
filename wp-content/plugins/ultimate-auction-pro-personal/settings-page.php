	...
	<form id="auction-settings-form" class="auction_settings_section_style" action="" method="POST">
	    ...
	    <?php
					...
					foreach ($options as $key => $option) {
						$err_msg = '';
						$values_by_user = get_user_meta($user_id, 'payment_options_enabled', true);
						$checked = (array_key_exists($key, $values_by_user)) ? ' checked="checked" ' : '';
                                                $values = get_option('payment_options_enabled');
						$disabled=(!array_key_exists($key, $values)) ? ' disabled="disabled" ' : '';
						if($disabled == ' disabled="disabled" '){
							$err_msg = '<span class="wdm-mark-red wdm-disabled-msg">'.__("disabled by Administrator", "wdm-ultimate-auction").'</span>';
						$checked='';	
						}
						echo "<div><input $checked $disabled value='$option' name='payment_options_enabled[$key]' type='checkbox' /> $option $err_msg </div>";
					}

					//echo "<br /><br />";
					
					//_e("NOTE: You cannot activate the payment methods which are disabled by Administrator.", "wdm-ultimate-auction");
	...
	<?php
		...
		public function wdm_page_init() {
			...
			if (isset($_GET["page"]) && $_GET["page"] == "ultimate-auction" && isset($_GET["setting_section"]) && $_GET["setting_section"] == "auction") {
				...
				add_settings_field(
					'wdm_auction_approve', 
					__('Do you want user auctions to be approved before going live?', 'wdm-ultimate-auction'), 
					array($this, 'wdm_auction_approval'), 
					'test-setting-admin', 
					'setting_section_id' 			
         			    );
		...
		public function wdm_validate_save_data($input) {
			...
			if (isset($_POST['ua_wdm_setting_auc']) && wp_verify_nonce($_POST['ua_wdm_setting_auc'], 'ua_setting_wp_n_f')) {
				...
				if(isset($mid['wdm_auction_approve']))
					update_option('wdm_auction_approve',$mid['wdm_auction_approve']);
				...
				if(isset($mid['wdm_enable_shipping'])){
					...
					update_option('wdm_enable_shipping','');
				}
...
echo '<br /><br /><div class="ult-auc-settings-tip">' . __("PRO has ability to add shipping/postage cost to auctions. It requires to collect postal address for registered users. When you enable this option, PRO adds postal address fields like street address, city, country and phone number inside default WP register form. So, visitors will now register to your site by entering their postal address. When they bid and win the auction, PRO would automatically calculate postage cost based on their country and add that to final payable cost for winning the auction.", "wdm-ultimate-auction") . '</div>';
		...
		public function wdm_set_payment_options() {
			...
			_e("If you choose to activate any payment method, please go to Payment tab and enter its details. For example: if you enable Wire Transfer, go to Payment -> Wire Transfer and enter its details. Same would apply to PayPal, Cheque and Cash.", "wdm-ultimate-auction");

			echo '<br/><br/>';
			
			echo '<strong>'.__("PLEASE NOTE: Only the enabled methods from above will be active for other sellers.", "wdm-ultimate-auction").'</strong>';
		}
...
public function wdm_auction_approval(){

    $default_val = get_option('wdm_auction_approve');
    
    $yes_select = $default_val == "yes" ? "selected='selected'" : "";
    
    $no_select = $default_val == "no" ? "selected='selected'" : "";
    
    echo "<select class='wdm_settings_input' id='wdm_auction_approve' name='wdm_auc_settings_data[wdm_auction_approve]'>";
    
    echo "<option value='no' $no_select>".__('No', 'wdm-ultimate-auction')."</option>";
	
    echo "<option value='yes' $yes_select>".__('Yes', 'wdm-ultimate-auction')."</option>";
