...
function wdm_ua_show_breadcrumb($brd_crmb, $auc_id){
	...
	$url = '';
	$url = get_option('wdm_auction_page_url');
	
	global $wpdb;
	if(!empty($url))
		$feed_page_title = $wpdb->get_var( $wpdb->prepare( "SELECT post_title FROM $wpdb->posts WHERE guid=%s", $url ) );
	
	if(empty($feed_page_title)){
		$ua_pages = get_option('ua_page_exists');
		
		if(isset($ua_pages['listing']))
			$feed_page_title = get_the_title($ua_pages['listing']);
	}
	...
	$brd_crmb = '<a class="ua_breadcrumb_link" href="'.$url.'">'.$feed_page_title.'</a>'.$brd_crmb.$symb.'<span class="ua_breadcrumb_link">'.get_the_title($auc_id).'</span>';
