...
class User_Auctions_List_Table extends WP_List_Table {        
	...
	$auction_user = get_option('wdm_usr_auctions_list');
		...
		if(!empty($invoiceStat) || get_post_meta($single_auction->ID,'auction_bought_status',true) === 'bought' || (!empty($winner) && in_array('administrator', $winner->roles))){
			...
			'auc_bid' => $amt_pay_to_user,
			'auc_seller' => $user_paypal_id,
			'auc_payer' => $merchant_email,
			'auc_currency' => $currency_code,
			'auc_url' => admin_url('admin.php?page=manage-user-auctions&auction_type=expired&auc='.$auth_key),
			'pay_type' => 'simple'/*,
	...
	if($this->auction_type === "pending"){
		...
		window.location.href = "<?php echo admin_url('admin.php?page=manage-user-auctions&auction_type=pending');?>";
		});
	    </script>
	    <?php
	    }
    ...
    if($this->auction_type=="live" || $this->auction_type=="pending")
    $columns =   array(
    ...
    //'current_price' => __('Starting / Current Price', 'wdm-ultimate-auction'),
    //'bidders'   => __('Bids Placed', 'wdm-ultimate-auction'),
    ////'status'   => __('Status', 'wdm-ultimate-auction'),
    //'action'    => __('Actions', 'wdm-ultimate-auction')
    //);
    else
    $columns =   array(
...
<?php
    ...
    $auc_stat=get_option("wdm_auction_approve");
    
    if($auc_stat=='yes')
    {
	?>	
	<li>|<a href="?page=manage-user-auctions&auction_type=pending" class="<?php echo $manage_auction_tab == 'pending' ? 'current' : ''; ?>">	<?php _e('Pending Auctions', 'wdm-ultimate-auction');?></a></li>	
    <?php
    }
    ?>
</ul>
