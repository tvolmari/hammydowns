...
$wdm_email_tabs = array(
	...
	array('slug' => 'best_offer_receive',
		'label' => __('Best Offer Receive', 'wdm-ultimate-auction'),
		'subject_hint' => __("Enter the subject of the 'Best Offer Receive' email. Available template tags:-\\n
                        {site_name} - The name of the site\\n
                        {product_name} - Auction item name", 'wdm-ultimate-auction'),
		'body_hint' => __("Enter the 'Best Offer Receive' email. Available template tags:-\\n
                        {product_url} - Auction item page URL\\n
                        {product_name} - Auction item name\\n
                        {currency_code} - The currency code\\n
                        {offer_value} - The currency code\\n
                        {product_description} - Auction item description\\n
                        {offer_sender_name} - The offer sender's name\\n
						{offer_sender_email} - The offer sender's email", 'wdm-ultimate-auction'),
	),
	array('slug' => 'best_offer_reject',
		'label' => __('Best Offer Reject', 'wdm-ultimate-auction'),
		'subject_hint' => __("Enter the subject of the 'Best Offer Reject' email. Available template tags:-\\n
                        {site_name} - The name of the site\\n
                        {product_name} - Auction item name", 'wdm-ultimate-auction'),
		'body_hint' => __("Enter the 'Best Offer Reject' email. Available template tags:-\\n
                        {product_url} - Auction item page URL\\n
                        {product_name} - Auction item name\\n
                        {currency_code} - The currency code\\n
                        {offer_value} - The currency code\\n
...
if (is_array($email_tabs)) {
	foreach ($email_tabs as $email_tab) {
		if ((!isset($_GET['email_type']) && $email_tab['slug'] == 'bid_place_bidder') || (isset($_GET['email_type']) && $_GET['email_type'] == $email_tab['slug'])) {
			...
			$wdm_fields = array(
				...
				$email_tab['slug'] . '[wdm_ua_email_template]' => array(
					...
					'value' => isset($get_email_template_details['template']) ? $get_email_template_details['template'] : "",
				),
...
foreach ($wdm_fields as $wdm_field => $wdm_data) {
				$type = isset( $wdm_data['type'] ) ? $wdm_data['type'] : "";
				$title = isset( $wdm_data['title'] ) ? $wdm_data['title'] : "";
				$description = isset( $wdm_data['description'] ) ? $wdm_data['description'] : "";
				$id = isset( $wdm_data['id'] ) ? $wdm_data['id'] : "";
				$css = isset( $wdm_data['css'] ) ? $wdm_data['css'] : "";
				$default = isset( $wdm_data['default'] ) ? $wdm_data['default'] : "";
				$value = isset( $wdm_data['value'] ) ? $wdm_data['value'] : "";
				$placeholder = isset( $wdm_data['placeholder'] ) ? $wdm_data['placeholder'] : "";
				$class = isset( $wdm_data['class'] ) ? $wdm_data['class'] : "";
				$options = isset( $wdm_data['options'] ) ? $wdm_data['options'] : array();
				
				if (in_array($type, array("text", "number", "checkbox"))) {
					wdm_ua_get_text_field($wdm_field, $title, $description, $id, $type, $css, $default, $placeholder, $value, $class);
				} else if ($type == "radio") {
					wdm_ua_get_radio_field($wdm_field, $title, $description, $id, $type, $css, $default, $options, $value, $class);
				} else if (isset($wdm_data['type']) && $wdm_data['type'] == "wp_editor") {
					wdm_ua_get_wp_editor_field($title, $description, $value, $id, $wdm_field);
				}
			}?>
		</table>
            <?php
...
function wdm_ua_get_text_field($wdm_field, $title, $desc, $id, $type, $css, $default, $placeholder, $value, $class) {
	?>
    <tr valign="top">
        <th scope="row"><?php echo $title;?></th>
        <td>
            <input class="input-text regular-input <?php echo esc_attr($class);?>" type="<?php echo esc_attr($type);?>" name="<?php echo esc_attr($wdm_field);?>" id="<?php echo esc_attr($id);?>" style="<?php echo esc_attr($css);?>" value="<?php echo $value;?>" <?php echo ((($value == "yes")||(empty($value)))? 'checked="checked"' : "");?> placeholder="<?php echo esc_attr($placeholder);?>" />
...
function wdm_ua_get_radio_field($wdm_field, $title, $desc, $id, $type, $css, $default, $options, $set_val, $class) {
	?>
    <tr valign="top">
        <th scope="row"><?php echo $title;?></th>
        <td>
...
$default = (!empty($set_val) ? $set_val : $default);
	...
	foreach ($options as $value => $display_name) {
		?>
            <input class="input-text regular-input <?php echo esc_attr($class);?>" type="<?php echo esc_attr($type);?>" name="<?php echo esc_attr($wdm_field);?>" id="<?php echo esc_attr($id);?>" style="<?php echo esc_attr($css);?>" value="<?php echo $value;?>" <?php echo (($default == $value) ? 'checked="checked"' : "");?> /> <?php echo $display_name;?>
