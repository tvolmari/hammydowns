...
class Auctions_List_Table extends WP_List_Table {        
	...
	elseif(isset($_GET["auction_type"]) && $_GET["auction_type"] == "pending" && !current_user_can('administrator'))
        {
            $args = array(
                'posts_per_page'  => -1,
                'post_type'       => 'ultimate-auction',
...
<ul class="subsubsub">
	...
	if($auc_stat=="yes" && !current_user_can('administrator'))
	{	
	    ?>
	    <li>|<a href="?page=manage_auctions&auction_type=pending" class="<?php echo $manage_auction_tab == 'pending' ? 'current' : ''; ?>"><?php _e('Pending Auctions', 'wdm-ultimate-auction');?></a></li>
	    <?php	
	}
    ?>
</ul>
