...
$edd_updater = new EDD_SL_Plugin_Updater(EDD_UAUCTION_STORE_URL, __FILE__, array(
	'version' => '4.5.2', // current version number
...
add_action('wp', 'wdm_check_immediate_auction_won');

function wdm_check_immediate_auction_won() {
	global $post;

	if ((isset($_GET['wdm']) && !empty($_GET['wdm'])) || (isset($_GET['wdmpy']) && !empty($_GET['wdmpy']))) {
		if ($post->post_type == 'ultimate-auction') {

			$single_auction = get_post(get_the_ID());
			...
			if (isset($_GET['wdm']) && $_GET['wdm'] === $auth_key) {
				...
				$return_url = get_permalink($single_auction->ID);
...
function prepare_single_auction_title($id, $title) {
	...
	//if (!empty($auc_url)) {
		$link_title = get_post_permalink($id);
...
function wdm_prepare_front_items($data_array, $col_array, $sh, $on) {
  ...
  <script type="text/javascript">
    jQuery(document).ready(function($){
       $('.wdm_front_live li, .wdm_front_expired li, .wdm_front_scheduled li, .wdm_front_pending li').mouseover(function(){$(this).find('.wdmua_action').css('visibility', 'visible')});
       $('.wdm_front_live li, .wdm_front_expired li, .wdm_front_scheduled li, .wdm_front_pending li').mouseout(function(){$(this).find('.wdmua_action').css('visibility', 'hidden')});
...
function ua_bst_ofrs_user_meta_callback() {
	...
	//update offers
    $wdm_auction_id = $_POST[ 'auction_id' ];
    $currency_code = substr(get_option('wdm_currency'), -3);
    $wdm_ua_offers_arr = get_post_meta( $wdm_auction_id, 'auction_best_offers', true );
    ...
    update_post_meta( $wdm_auction_id, 'auction_best_offers', $wdm_ua_offers_arr );

    $wdm_auction = get_post( $wdm_auction_id );

    $ret_url = get_post_meta( $wdm_auction_id, "current_auction_permalink", true );

    $offer_sender = new WP_User($_POST[ 'os_id' ]);
	$offer_sender_name = $offer_sender->user_login;
	$offer_sender_email = $offer_sender->user_email;

    $wdm_mail_sent = ua_place_best_offers_mail( esc_js($wdm_auction->post_title), $wdm_auction_id, esc_js($wdm_auction->post_content), round($_POST[ 'offer_val' ], 2), $offer_sender_name, $offer_sender_email, $ret_url);
    
    $notify_msg = '';
    
    if($wdm_mail_sent)
	$notify_msg = '<p style="color:green;">'.__('A notification has been sent to this item owner.', 'wdm-ultimate-auction').'</p>';
    else
	$notify_msg = '<p style="color:red;">'.__('Error in sending the notification to the item owner.', 'wdm-ultimate-auction').'</p>';
	
    echo '<br /><div class="wdm_offers_sent_text">
                <p>'. __('Your offer is pending for review by auction owner.', 'wdm-ultimate-auction').'</p>
		<p>'.sprintf(__('Offer amount: %s%s', 'wdm-ultimate-auction'), $currency_code." ", $_POST[ 'offer_val' ]).'</p>'.$notify_msg.'
                </div>
                <br />';
...
function wdm_best_offer_owner_action_callback() {
	...
	if( $_POST['owner_decision'] == "reject" ) {
		...
		if( isset( $auction_best_offers_arr[ $_POST[ 'wdm_best_offer_sender_id' ] ] ) ) {
			...
			$wdm_auction = get_post( $wdm_auction_id );
			$ret_url = get_post_meta( $wdm_auction_id, "current_auction_permalink", true );
			$wdm_mail_sent = ua_best_offers_reject_mail( esc_js($wdm_auction->post_title), $wdm_auction_id, esc_js($wdm_auction->post_content), round($_POST[ 'wdm_best_offer_val' ], 2), $_POST['wdm_best_offer_sender_email'], $ret_url);
			echo json_encode(array('status' => 'success', 'email_status' => $wdm_mail_sent) );
		}
...
function wdm_download_digital_product_file() {
	...
	if ( isset($post->post_type) && ($post->post_type == 'ultimate-auction') && isset($_GET['url']) && !empty($_GET['url'])) {
...
add_action('wp', 'wdm_download_digital_product_file');
...
//
//     if ($post->post_type == 'ultimate-auction') {
//          $single_template = dirname( __FILE__ ) . '/ultimate-auction-template.php';
//     }
//     return $single_template;
//}
//add_filter( 'single_template', 'get_custom_post_type_template' );

function wdm_get_auction_content($content){
	...
	if ($post->post_type == 'ultimate-auction') {
		...
		require(dirname( __FILE__ ) . '/ultimate-auction-template.php');
	}
	
	return $content;
}

add_filter( 'the_content', 'wdm_get_auction_content', 9 );

function wdm_ua_share_og_tag(){
	global $post;
	//$ua_featured_img = wp_get_attachment_url( get_post_thumbnail_id( $post->ID ) );
	$ua_featured_img = get_post_meta($post->ID, 'wdm_auction_thumb', true);
	$ua_featured_desc = $post->post_excerpt; 
	if ($post->post_type == 'ultimate-auction') {
		remove_filter('the_content', 'wpautop');
		remove_filter('the_content', 'wptexturize');
		remove_filter('the_content', 'convert_chars');
        ?>
          <meta property="og:image" content="<?php echo $ua_featured_img;?>"/>
          <meta property="og:description" content="<?php echo $ua_featured_desc;?>" />
        <?php
     }
}

add_action( 'wp_head', 'wdm_ua_share_og_tag' );

function wdm_ult_auc_page_redirect(){

	$ult_auc_url_stat = false;
	$ult_auc_id = "";
	$url_param = $_GET;
	foreach ($url_param as $parameter => $value) {
		if( $parameter === 'ult_auc_id' ){
			$ult_auc_url_stat = true;
			$ult_auc_id = $value;
			unset( $url_param[ $parameter ] );
		}
	}

	if( $ult_auc_url_stat ){
		$ult_auc_url = get_permalink( $ult_auc_id );
		$ult_auc_url = add_query_arg( $url_param, $ult_auc_url );
		
		wp_redirect( $ult_auc_url ); exit;
	}
}

add_action( 'init', 'wdm_ult_auc_page_redirect' );

function wdm_ua_buy_now_adaptive(){
	
	require_once('ua-paypal-invoice/paypal-adaptive/UA-ParallelPay.php');
	die();
}

add_action( 'wp_ajax_wdm_ua_buy_now_adaptive', 'wdm_ua_buy_now_adaptive' );
add_action( 'wp_ajax_nopriv_wdm_ua_buy_now_adaptive', 'wdm_ua_buy_now_adaptive' );
?>