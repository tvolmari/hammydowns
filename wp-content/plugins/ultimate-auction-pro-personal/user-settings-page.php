<?php
    ...
    $user_settings .= '<div style="float:left;width: 50%;">';
	...
	if(in_array('administrator', $logged_user_role)){
	    ...
	    $values_allowed = get_option('payment_options_enabled');
	    $disabled=(!array_key_exists($key, $values_allowed)) ? ' disabled="disabled" ' : '';
	    $checked = (array_key_exists($key, $values)) ? ' checked="checked" ' : '';
	}
	...
	$user_settings .=  "<div><input $checked  $disabled value='$option' name='payment_options_enabled[$key]' type='checkbox' /> $option $err_msg </div>";
    }
    $user_settings .= '</div>';
    
    $user_settings .=  "<br /><br />";
    //$user_settings .= '<div style="width:100%;float:left;margin:5px;">'.__("NOTE: You cannot activate the payment methods which are disabled by Administrator.", "wdm-ultimate-auction").'</div>';
    //$user_settings .=  "<br /><br />";
    ...
    if(in_array('administrator', $logged_user_role))
	$user_settings .= '<strong>'.__("PLEASE NOTE: Only the enabled methods from above will be active for other sellers.", "wdm-ultimate-auction").'</strong>';
