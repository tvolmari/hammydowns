...
if (!empty($_POST)) {
	if (isset($_POST['ua_wdm_add_auc']) && wp_verify_nonce($_POST['ua_wdm_add_auc'], 'ua_wp_n_f')) {
		...
		$def_auc_stat = get_option("wdm_auction_approve");
		
		if($def_auc_stat=="yes" && !in_array('administrator', $logged_user_role)){
		    $auction_stat = "pending";
		}
		elseif($can_add_directly) {
			...
			if ($post_id) {
				...
				for($im = 1; $im <= 4; $im++) {
					if (get_post_meta($post_id, 'wdm-main-image', true) == 'main_image_' . $im) {
						...
						//$ua_thumb_id = $wpdb->get_var( $wpdb->prepare( "SELECT ID FROM $wpdb->posts WHERE guid=%s", $main_image ) );
						//update_post_meta($post_id, '_thumbnail_id', $ua_thumb_id);
					}
				}
...
if (!empty($bidding_engine)) {
	...
	if ((get_option("wdm_default_bidding_engine_set") == "Yes") || (get_option("wdm_default_bidding_engine_set") == "No" && get_option("wdm_bidding_engines") == "") || (get_option("wdm_default_bidding_engine_set") == "")) {
...
echo "<div id='auction_image_gallery'>" . $med_up . "</div>";?>
    ...
    <tr valign="top" class="standard_auction_section">
	    ...
	    <a href="" class="auction_fields_tooltip" onclick="return false"><strong><?php _e("?", "wdm-ultimate-auction");?></strong>
...
if (in_array('administrator', $logged_user_role)) {
	...
	$pay_methods_allowed=get_option('payment_options_enabled');
	$pay_methods=array_intersect($pay_methods,$pay_methods_allowed);
}
...
?>><?php _e("PayPal", "wdm-ultimate-auction");?></option>
	...
	echo "<span class='wdm-mark-red'>" . __("You'll be able to choose only PayPal for getting the payment as admin has activated commissions feature.", "wdm-ultimate-auction") . "</span>";
	    ...
	    <div class="ult-auc-settings-tip"><?php 
	    _e("Only those methods will be active which you have selected on 'Settings' page.", "wdm-ultimate-auction");?></div>
        </td>
    </tr>
    </table>
    <?php
	...
	if ($can_add_directly && !isset($_GET['edit_auction']) && !isset($_GET["reactivate"])) {submit_button(__('Save Changes', 'wdm-ultimate-auction'));} elseif ($can_add_directly && isset($_GET["reactivate"])) {submit_button(__('Save Changes', 'wdm-ultimate-auction'));} elseif (isset($_GET['edit_auction']) && !empty($_GET['edit_auction']) && !isset($_GET["reactivate"])) {
		...
		if (isset($_GET["tx"]) && !empty($_GET["tx"])) {
			...
			if (isset($_GET["auc"]) && $_GET["auc"] === $auth) {
				...
				if(get_post_status( $_GET['edit_auction'] ) == 'draft')
				    wp_update_post($args);
