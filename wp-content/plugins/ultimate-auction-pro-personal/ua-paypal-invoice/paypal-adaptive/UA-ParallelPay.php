...
if(isset($_POST['auc_data']))
	$auc_data = $_POST['auc_data'];

if(isset($_POST['auc_data_adm']))
	$auc_data = json_decode(base64_decode($_POST['auc_data_adm']), TRUE);
...
if(isset($auc_data['pay_type']) && $auc_data['pay_type'] == 'simple'){
	$primaryReceiver[0] = false;
}
else{
...
if($ack != "SUCCESS") {
	...
	if(isset($_POST['adp_bin_btn'])){ 
			echo json_encode(array('payment_status' => 'failure', 'data' => __('Failed to connect with PayPal.', 'wdm-ultimate-auction').$err_msg));
	...
	if($shipAmt > 0){
		...
		if(isset($_POST['adp_bin_btn'])){
		if($ack1 == "SUCCESS") {
			echo json_encode(array('payment_status' => 'success', 'data' => $payPalURL));
			}
		}
	}
	else{
